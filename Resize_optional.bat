...
:: Lặp qua tất cả các định dạng ảnh phổ biến (dùng dấu ngoặc kép để xử lý tên file có khoảng trắng)
for %%F in ("%folder%\*.jpg" "%folder%\*.jpeg" "%folder%\*.png" "%folder%\*.webp" "%folder%\*.bmp") do (
    if exist "%%F" (
        magick "%%F" ^
